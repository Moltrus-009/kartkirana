...
import { appCheck } from '../infrastructure/firebase/firebase';
...
const initializeNativeAppCheck = (): Promise<void> => {
  if (!nativeInitialization) {
    nativeInitialization = FirebaseAppCheck.initialize({
      ...
      // Sideloaded debug APKs cannot rely on Play-distribution attestation.
      // Enable the debug provider only in the separately built internal APK;
      // release AAB builds leave this variable unset and use Play Integrity.
...
export const getSecureAppCheckToken = async (forceRefresh = false): Promise<string> => {
  ...
  if (!appCheck) {
    throw new Error('Web App Check is not configured for this build.');
  }
